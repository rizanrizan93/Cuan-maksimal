...
USER_AGENT = "Mozilla/5.0 (compatible; IDX-Emir-Autonomous-Scanner/1.9.1; research)"
...
def parse_ksei_price_history_html(html: str) -> pd.DataFrame:
    ...
    for table in soup.find_all("table"):
        ...
        for row in table_rows[1:]:
            ...
            date = parse_public_date(values.get("date"))
            if pd.isna(date):
                continue
...
def parse_universe_frame(uploaded: Any) -> pd.DataFrame:
    ...
    aliases = {
        ...
        "industry": "sector", "sektor": "sector", "sector_idx_ic": "sector", "idx_ic_sector": "sector",
