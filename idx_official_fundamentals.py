...
def parse_idx_xbrl_instance(ticker: str, xml_bytes: bytes, *, source_url: str = "", period_label: str = "") -> dict[str, Any]:
    ...
    if not np.isfinite(ocf):
        cash_quality_state = "CASHFLOW_NOT_AVAILABLE"
    elif ocf < 0:
        cash_quality_state = "OCF_AND_FCF_NEGATIVE" if np.isfinite(fcf) and fcf < 0 else "OCF_NEGATIVE"
    elif np.isfinite(fcf) and fcf < 0:
        cash_quality_state = "FCF_NEGATIVE_CAPEX_REVIEW"
    elif np.isfinite(ocf_conversion) and ocf_conversion < 0.50:
        cash_quality_state = "CASHFLOW_POSITIVE_WEAK_CONVERSION"
    elif np.isfinite(ocf_conversion) and ocf_conversion >= 0.70:
        cash_quality_state = "CASHFLOW_POSITIVE_CONVERTING"
    else:
        cash_quality_state = "CASHFLOW_POSITIVE_PARTIAL"
    ...
    return {
        ...
        "idx_official_cashflow_quality_state": cash_quality_state,
