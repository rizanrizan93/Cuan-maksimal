...
IDX_OFFICIAL_FUNDAMENTAL_TTL_HOURS = 48.0
MARKET_FEATURE_CACHE_FAMILY = "MARKET_FEATURES"
MARKET_FEATURE_CACHE_SCHEMA_VERSION = "1"
MARKET_FEATURE_CACHE_TTL_HOURS = 72.0
DB_READ_CHUNK_SIZE = 100
...
_FUNDAMENTAL_REQUIRED_KEYS = {
    ...
    "interest_bearing_debt_to_equity", "total_liabilities_to_equity",
    "net_debt_to_equity", "current_ratio", "cash_to_debt_ratio",
    "fundamental_period_alignment_state", "fundamental_cashflow_state",
...
def _expected_quarter_end(now: Any = None) -> pd.Timestamp | pd.NaT:
    """Conservative reporting-season expectation used only to bypass stale cache hits."""
...
def _fundamental_payload_reporting_lagged(payload: Mapping[str, Any] | None, now: Any = None) -> bool:
    expected = _expected_quarter_end(now)
    if pd.isna(expected) or not isinstance(payload, Mapping):
        return False
    observed = pd.to_datetime(payload.get("fundamental_latest_period"), errors="coerce")
    if pd.isna(observed):
        return False
    return bool(pd.Timestamp(observed).normalize() < pd.Timestamp(expected).normalize())
...
def _get_in_chunks(config: DatabaseConfig, table: str, key: str, values: Iterable[str], select: str = "*", chunk_size: int = DB_READ_CHUNK_SIZE) -> list[dict[str, Any]]:
    unique = list(dict.fromkeys(str(value) for value in values if str(value)))
    rows: list[dict[str, Any]] = []
    size = max(1, min(int(chunk_size or DB_READ_CHUNK_SIZE), 200))
...
def _ohlcv_cache_fresh(
    row: Mapping[str, Any], frame: pd.DataFrame, *, ttl_hours: float, now: Any = None,
    require_latest_session: bool = True,
) -> bool:
    if frame.empty:
        return False
    # In completed-session production mode, cache age alone is insufficient: a scan written before
    # the latest close can still be one session stale the next morning. Diagnostic/intraday mode
    # may intentionally accept a recent cache without requiring the latest completed session.
    if require_latest_session:
        expected = _expected_completed_weekday(now)
        latest = frame.index.max().normalize()
        if latest < expected:
            return False
    return _cache_age_hours(row, now) <= float(ttl_hours) or (require_latest_session and frame.index.max().normalize() >= _expected_completed_weekday(now))
...
def build_market_feature_cache_row(
    ticker: str,
    ...
    *,
    ...
    payload.update({
        "ticker": normalize_ticker(ticker),
        "market_feature_cache_schema_version": MARKET_FEATURE_CACHE_SCHEMA_VERSION,
        "market_feature_engine_version": MARKET_FEATURE_VERSION,
    })
    return build_source_cache_row(
        ticker,
        MARKET_FEATURE_CACHE_FAMILY,
        payload,
        provider="DERIVED_FROM_OHLCV",
        status="OK" if str(payload.get("feature_state") or "") == "OK" else "INVALID",
        checked_at=checked_at,
        ttl_hours=MARKET_FEATURE_CACHE_TTL_HOURS,
        latest_observed_at=payload.get("last_date"),
        last_scan_id=last_scan_id,
    )


def _market_feature_payload_compatible(payload: Any, now: Any = None, expected_session_date: Any = None) -> bool:
    if not isinstance(payload, Mapping) or not payload:
        return False
    if str(payload.get("market_feature_cache_schema_version") or "") != MARKET_FEATURE_CACHE_SCHEMA_VERSION:
        return False
    if str(payload.get("market_feature_engine_version") or "") != MARKET_FEATURE_VERSION:
        return False
    if str(payload.get("feature_state") or "") != "OK":
        return False
    last_date = pd.to_datetime(payload.get("last_date"), errors="coerce")
    expected = pd.to_datetime(expected_session_date, errors="coerce") if expected_session_date is not None else _expected_completed_weekday(now)
    if pd.isna(last_date) or pd.isna(expected):
        return False
    return bool(pd.Timestamp(last_date).tz_localize(None).normalize() >= pd.Timestamp(expected).tz_localize(None).normalize())


def load_cached_market_features(
    config: DatabaseConfig,
    tickers: Iterable[str],
    *,
    now: Any = None,
    force_refresh: bool = False,
    expected_session_date: Any = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    symbols = list(dict.fromkeys(normalize_ticker(ticker) for ticker in tickers if normalize_ticker(ticker)))
    if force_refresh or not symbols:
        audit = pd.DataFrame([{
            "ticker": symbol, "provider": "SUPABASE_MARKET_FEATURE_CACHE", "status": "FORCE_REFRESH",
            "items": 0, "detail": "Feature cache bypassed.", "cache_state": "MARKET_FEATURE_CACHE_BYPASS",
        } for symbol in symbols])
        return pd.DataFrame(), audit
    rows = read_source_cache(config, symbols, MARKET_FEATURE_CACHE_FAMILY)
    records: list[dict[str, Any]] = []
    audits: list[dict[str, Any]] = []
    for symbol in symbols:
        row = rows.get(symbol)
        payload = row.get("payload") if isinstance(row, Mapping) else None
        hash_ok = bool(row and _row_hash_valid(row))
        compatible = bool(hash_ok and _market_feature_payload_compatible(payload, now=now, expected_session_date=expected_session_date))
        if compatible:
            record = dict(payload)
            record["ticker"] = symbol
            records.append(record)
            audits.append({
                "ticker": symbol, "provider": "SUPABASE_MARKET_FEATURE_CACHE", "status": "CACHE_HIT",
                "items": 1, "detail": f"last_date={record.get('last_date')}", "cache_state": "MARKET_FEATURE_CACHE_HIT",
            })
        else:
            reason = "MISSING" if not row else "HASH_INVALID" if not hash_ok else "STALE_OR_INCOMPATIBLE"
            audits.append({
                "ticker": symbol, "provider": "SUPABASE_MARKET_FEATURE_CACHE", "status": "CACHE_MISS",
                "items": 0, "detail": reason, "cache_state": "MARKET_FEATURE_CACHE_MISS",
            })
    frame = pd.DataFrame(records)
    if not frame.empty and "ticker" in frame.columns:
        frame = frame.drop_duplicates("ticker", keep="last").reset_index(drop=True)
...
def fetch_ohlcv_cache_first(
    ...
    *,
    ...
    for symbol in symbols:
        ...
        if row and period_sufficient and not force_refresh and _ohlcv_cache_fresh(row, cached, ttl_hours=cache_ttl_hours, now=now, require_latest_session=completed_only):
...
def fetch_fundamental_cache_first(
    ...
    *,
    ...
    memory = load_latest_research_memory(config, symbols, "PUBLIC_FUNDAMENTAL_PROXY", limit_per_ticker=1)
    ...
    for symbol in symbols:
        ...
        if row and not _fundamental_payload_compatible(payload):
            # Never reuse pre-v1.7 fundamental payloads after YoY/QoQ/TTM schema changes.
            cached.pop(symbol, None)
            row = None
            payload = {}
        ...
        reporting_lagged = _fundamental_payload_reporting_lagged(payload, now)
        if row and not force_refresh and _source_row_fresh(row, now) and not reporting_lagged:
            if payload:
            ...
            if row and reporting_lagged and not force_refresh:
                audits.append({"ticker": symbol, "provider": "SUPABASE_FUNDAMENTAL_CACHE", "status": "REPORTING_PERIOD_LAG_REFRESH", "items": 0, "detail": "Cached statement is behind the expected quarterly reporting period; provider refresh forced.", "cache_state": "REPORTING_PERIOD_LAG_REFRESH"})
            refresh.append(symbol)
    writes: list[dict[str, Any]] = []
    if refresh:
...
def fetch_idx_official_fundamental_cache_first(
    config: DatabaseConfig, tickers: Iterable[str], *, max_workers: int = 2, now: Any = None,
    force_refresh: bool = False, last_scan_id: str = "",
) -> tuple[pd.DataFrame, pd.DataFrame, list[dict[str, Any]]]:
    symbols=list(dict.fromkeys(normalize_ticker(t) for t in tickers if normalize_ticker(t)))
    cached=read_source_cache(config,symbols,"IDX_FUNDAMENTAL")
    memory=load_latest_research_memory(config,symbols,"IDX_OFFICIAL_FUNDAMENTAL",limit_per_ticker=1)
    snapshots=[]; audits=[]; refresh=[]; writes=[]
    for symbol in symbols:
        row=cached.get(symbol)
        if row and not _row_hash_valid(row): row=None
        payload=row.get("payload") if row and isinstance(row.get("payload"),dict) else {}
        if row and not force_refresh and _source_row_fresh(row,now):
            if payload and bool(payload.get("idx_official_source_verified")): snapshots.append({"ticker":symbol,**payload})
            audits.append({"ticker":symbol,"provider":"SUPABASE_IDX_OFFICIAL_CACHE","status":"CACHE_HIT","items":int(bool(payload.get("idx_official_source_verified"))),"detail":f"age_hours={_cache_age_hours(row,now):.2f}","cache_state":"CACHE_HIT"})
        else: refresh.append(symbol)
    if refresh:
        fresh, fa=fetch_many_idx_official_fundamentals(refresh,now=now,max_workers=max_workers)
        fmap=fresh.set_index("ticker").to_dict(orient="index") if not fresh.empty else {}
        amap={str(r.get("ticker")):r for r in fa.to_dict(orient="records")} if not fa.empty else {}
        for symbol in refresh:
            payload={"ticker":symbol,**fmap.get(symbol,{})}; audit=amap.get(symbol,{})
            verified=bool(payload.get("idx_official_source_verified"))
            if verified: snapshots.append(payload)
            else:
                remembered=(memory.get(symbol) or [{}])[0].get("payload") if memory.get(symbol) else {}
                if isinstance(remembered,dict) and bool(remembered.get("idx_official_source_verified")):
                    snapshots.append({"ticker":symbol,**remembered})
                    audits.append({**audit,"ticker":symbol,"provider":"SUPABASE_RESEARCH_MEMORY","status":"RESEARCH_MEMORY_FALLBACK","items":1,"cache_state":"RESEARCH_MEMORY_FALLBACK"})
            if not (not verified and any(str(a.get("status"))=="RESEARCH_MEMORY_FALLBACK" and a.get("ticker")==symbol for a in audits)):
                audits.append({**audit,"ticker":symbol,"status":str(audit.get("status") or ("OK" if verified else "NO_ITEMS")),"cache_state":"COLD_REFRESH" if symbol not in cached else "REFRESHED"})
            # Negative result is cached briefly too, preventing repeated hammering of IDX.
            writes.append(build_source_cache_row(symbol,"IDX_FUNDAMENTAL",payload,provider="IDX_OFFICIAL_XBRL",status="OK" if verified else "NO_ITEMS",checked_at=now,ttl_hours=IDX_OFFICIAL_FUNDAMENTAL_TTL_HOURS,last_scan_id=last_scan_id))
    return pd.DataFrame(snapshots),pd.DataFrame(audits),writes


def load_cached_idx_official_fundamentals(config: DatabaseConfig, tickers: Iterable[str]) -> tuple[pd.DataFrame,pd.DataFrame]:
    symbols=list(dict.fromkeys(normalize_ticker(t) for t in tickers if normalize_ticker(t)))
    rows=read_source_cache(config,symbols,"IDX_FUNDAMENTAL")
    memory=load_latest_research_memory(config,symbols,"IDX_OFFICIAL_FUNDAMENTAL",limit_per_ticker=1)
    snapshots=[]; audits=[]
    for symbol in symbols:
        row=rows.get(symbol); payload=row.get("payload") if row and _row_hash_valid(row) and isinstance(row.get("payload"),dict) else {}
        if payload and bool(payload.get("idx_official_source_verified")):
            snapshots.append({"ticker":symbol,**payload}); audits.append({"ticker":symbol,"provider":"SUPABASE_IDX_OFFICIAL_CACHE","status":"CACHE_LOAD","items":1,"cache_state":"CACHE_LOAD"})
        else:
            remembered=(memory.get(symbol) or [{}])[0].get("payload") if memory.get(symbol) else {}
            if isinstance(remembered,dict) and bool(remembered.get("idx_official_source_verified")):
                snapshots.append({"ticker":symbol,**remembered}); audits.append({"ticker":symbol,"provider":"SUPABASE_RESEARCH_MEMORY","status":"RESEARCH_MEMORY_LOAD","items":1,"cache_state":"RESEARCH_MEMORY_LOAD"})
            else: audits.append({"ticker":symbol,"provider":"SUPABASE_IDX_OFFICIAL_CACHE","status":"CACHE_MISS","items":0,"cache_state":"CACHE_MISS"})
...
def persist_verify_cache_bundle(
    ...
    *,
    ...
    # v1.9.14 persists market-feature rows independently from OHLCV. Treat each
    # row family as optional so source-only / OHLCV-only checkpoints cannot fail
    # merely because an empty companion list was omitted by a caller.
    ohlcv_rows = list(ohlcv_rows or [])
...
__all__ = [
    "CACHE_VERSION", "FUNDAMENTAL_CACHE_SCHEMA_VERSION", "OHLCV_CACHE_TTL_HOURS", "KSEI_CACHE_TTL_HOURS", "FUNDAMENTAL_CACHE_TTL_HOURS", "NEWS_CACHE_TTL_HOURS", "IDX_OFFICIAL_FUNDAMENTAL_TTL_HOURS", "MARKET_FEATURE_CACHE_FAMILY", "MARKET_FEATURE_CACHE_SCHEMA_VERSION", "MARKET_FEATURE_CACHE_TTL_HOURS",
    "frame_to_payload", "payload_to_frame", "build_ohlcv_cache_row", "build_source_cache_row", "build_market_feature_cache_row", "load_cached_market_features", "read_ohlcv_cache", "read_source_cache",
    "fetch_ohlcv_cache_first", "fetch_ksei_cache_first", "fetch_fundamental_cache_first", "fetch_idx_official_fundamental_cache_first", "fetch_news_cache_first",
