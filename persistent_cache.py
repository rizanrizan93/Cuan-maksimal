...
def _get_in_chunks(config: DatabaseConfig, table: str, key: str, values: Iterable[str], select: str = "*") -> list[dict[str, Any]]:
    ...
    for start in range(0, len(unique), 20):
        ...
        if not chunk:
            continue
        response = _request(config, "GET", table, params={"select": select, key: "in.(" + ",".join(f'"{value.replace(chr(34), "")}"' for value in chunk) + ")"})
...
__all__ = [
    ...
    "persist_verify_cache_bundle", "cache_commit_succeeded", "cache_summary", "trim_period",
]
