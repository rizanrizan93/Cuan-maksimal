...
def compute_future_fundamental_grade(
    ...
    if not d.empty:
        ...
        obv_scale = 1.0
        try:
            obv_hist = d["OBV_SLOPE10"].astype(float).replace([np.inf, -np.inf], np.nan).dropna().tail(60)
            if len(obv_hist) > 0:
                obv_scale = float(np.nanmedian(np.abs(obv_hist)))
        except Exception:
            obv_scale = 1.0
        obv_norm = float(np.tanh(obv_slope / (obv_scale + 1e-9)))
