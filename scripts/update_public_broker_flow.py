...
def main() -> int:
    ...
    for offset in range(7):
        ...
        try:
            ...
            daily = trim_daily_top_flow(aggregate_trade_detail(path, candidate))
            if not daily.empty:
    ...
    print(f"Emir broker cache updated: source_date={source_date} rows={len(combined)} source={source_url}")
    return 0
