...
def _to_shared_rows(frame: pd.DataFrame, fetched_at: datetime) -> list[dict]:
    ...
    if normalized.empty:
        return []
    hash_columns = [name for name in ("ticker", "trade_date", "foreign_buy_shares", "foreign_sell_shares", "foreign_net_shares", "volume", "value", "source") if name in normalized.columns]
    ...
    for _, row in normalized.iterrows():
        ...
        rows.append({"provider": "ZAPI", "trade_date": pd.Timestamp(row["trade_date"]).date().isoformat(), "ticker": str(row["ticker"]), "foreign_buy_shares": _json_value(row.get("foreign_buy_shares")), "foreign_sell_shares": _json_value(row.get("foreign_sell_shares")), "foreign_net_shares": _json_value(row.get("foreign_net_shares")), "volume": _json_value(row.get("volume")), "value": _json_value(row.get("value")), "flow_unit": str(row.get("flow_unit") or "SHARES"), "source_family": source_family, "source_url": ZAPI_STOCK_SUMMARY_URL if "STOCK_SUMMARY" in source_family else ZAPI_FOREIGN_FLOW_URL, "payload_hash": payload_hash, "fetched_at": fetched_at.isoformat(), "freshness_state": "CURRENT", "validation_state": "VALID"})
    return rows
...
def _from_shared_rows(rows: list[dict] | tuple[dict, ...]) -> pd.DataFrame:
    if not rows:
        return _normalize_history(pd.DataFrame())
    return _normalize_history(pd.DataFrame(rows).rename(columns={"source_family": "source"}))


def _provider_reason(meta: dict) -> str:
    text = json.dumps(meta, sort_keys=True).upper()
    for reason in ("HTTP_401", "HTTP_403", "HTTP_404", "HTTP_429", "TIMEOUT", "CONNECTION_ERROR", "QUOTA_EXHAUSTED", "RATE_LIMIT"):
        if reason in text:
            return reason
    return MissingReason.PROVIDER_NO_DATA.value if str(meta.get("state") or "").upper() == "NO_DATA" else MissingReason.EMPTY_RESPONSE.value
...
def _shared_session(context, universe: list[str], cursor: pd.Timestamp):
    ...
    day, meta_box = cursor.date().isoformat(), {}
    def read_current():
        return backend.read_rows("evidence_foreign_flow", {"provider": "ZAPI", "trade_date": day}, limit=2000)
    def fetch():
    ...
    def persist(rows):
        return len(backend.upsert_rows("evidence_foreign_flow", rows, conflict=("provider", "trade_date", "ticker")))
    def validate(rows):
        minimum = max(1, int(len(universe) * 0.80))
        return len(rows) >= minimum, "VALID" if len(rows) >= minimum else _provider_reason(meta_box)
    result = coordinator.get_or_refresh(EvidenceKey("ZAPI", "FOREIGN_FLOW", "IDX_ALL", cursor.date()), read_current=read_current, fetch=fetch, persist=persist, validate=validate, minimum_rows=max(1, int(len(universe) * 0.80)), lease_seconds=300)
    meta = {"state": result.reason, "provider": "SHARED_IDX_EVIDENCE_HUB", "rows": len(result.rows), "provider_called": result.provider_called, "request_avoided": result.request_avoided, "lease_state": result.lease_state}
...
def main() -> int:
    ...
    if shared_context:
        state["shared_cache"] = shared_context[1].metrics()
    _write_state(state)
    if not parts:
