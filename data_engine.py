...
def _clean_ticker_text(symbol: str) -> str:
    s = str(symbol).strip().upper()
    if not s or s == "NAN":
        return ""
    # Remove common spreadsheet / copy-paste artifacts.
    s = s.strip().strip("'\"")
    while s.startswith("$"):
        s = s[1:]
    s = s.strip().strip("'\"")
...
def _ticker_candidates(symbol: str) -> list[str]:
    base = _clean_ticker_text(symbol)
    if not base:
        return []
    ...
    def add(candidate: str) -> None:
        candidate = _clean_ticker_text(candidate)
...
def load_yf_info(symbol: str) -> dict:
    ...
    crumb / 401 failures seen on the full quoteSummary endpoint. This path stays
    intentionally lightweight to reduce rate-limit pressure in Streamlit Cloud.
    """
    ...
    for candidate in _ticker_candidates(base):
        try:
            ...
            # Keep this path light-weight: fast_info is usually enough for the
            # app, and avoiding the heavy quoteSummary/info endpoints prevents
            # many 401 / Invalid Crumb failures on Streamlit Cloud.
            if merged:
                merged["_resolved_symbol"] = candidate
                merged["_info_source"] = "fast_info"
                return merged
        except Exception:
            continue
