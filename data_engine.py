...
_TICKER_OHLCV_CACHE: dict[tuple[str, int], tuple[float, pd.DataFrame]] = {}
_TICKER_OHLCV_CACHE_LOCK = threading.RLock()
_TICKER_OHLCV_TTL_SECONDS = 15 * 60


def _load_ticker_data_uncached(symbol: str, months: int) -> pd.DataFrame:
    """Load OHLCV history with conservative retries.

    The cache is intentionally memory-only and TTL-based so the scanner always
    refreshes after restart, while still avoiding repeated downloads inside a
    short scan window.
    """
    try:
        months = int(months)
...
def load_ticker_data(symbol: str, months: int) -> pd.DataFrame:
    """Return OHLCV data using a 15-minute in-memory cache.

    Cache entries are invalidated automatically after 15 minutes and are not
    persisted across restarts or reloads.
    """
    try:
        months = int(months)
    except Exception:
    ...
    cache_key = (sym, months)
    now = time.time()

    with _TICKER_OHLCV_CACHE_LOCK:
        cached = _TICKER_OHLCV_CACHE.get(cache_key)
        if cached is not None:
            cached_ts, cached_df = cached
            if now - cached_ts <= _TICKER_OHLCV_TTL_SECONDS and isinstance(cached_df, pd.DataFrame) and not cached_df.empty:
                return cached_df.copy()

    df = _load_ticker_data_uncached(sym, months)

    with _TICKER_OHLCV_CACHE_LOCK:
        if isinstance(df, pd.DataFrame) and not df.empty:
            _TICKER_OHLCV_CACHE[cache_key] = (now, df.copy())
        else:
            _TICKER_OHLCV_CACHE.pop(cache_key, None)
