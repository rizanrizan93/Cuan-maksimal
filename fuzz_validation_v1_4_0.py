...
def main() -> None:
    ...
    for i in range(2000):
        ...
        integrity_prov = rng.choice(["AUTO_PUBLIC_KSEI_PROXY", "AUTO_PUBLIC_KSEI_AND_REGULATORY_NEWS", "DIRECT_SOURCE_VERIFIED", "PROVIDER_FAILED"])
        integrity = {
        ...
        if profile.get("production_ready"):
            ...
            if tier == "DIRECT_PRECISE" and (integrity_prov != "DIRECT_SOURCE_VERIFIED" or orderbook_prov != "DIRECT_SOURCE_VERIFIED"):
                gate_bypass += 1
            if tier == "AUTO_EOD_PROXY" and (integrity_prov not in {"AUTO_PUBLIC_KSEI_PROXY", "AUTO_PUBLIC_KSEI_AND_REGULATORY_NEWS"} or orderbook_prov != "OHLCV_EOD_MICROSTRUCTURE_PROXY_NOT_LIVE_DEPTH"):
                gate_bypass += 1
    result = {
        "rows": 2000,
        "crashes": crashes,
    ...
    Path("validation_artifacts/FUZZ_VALIDATION_V1_4_0.json").write_text(json.dumps(result, indent=2) + "\n")
