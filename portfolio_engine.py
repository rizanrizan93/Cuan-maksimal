...
def init_store() -> None:
    ...
    with _connect() as conn:
        ...
        _ensure_table_columns(conn, "orders", {"target_1": "REAL", "target_2": "REAL"})
        cur.close()



def _ensure_table_columns(conn: Any, table_name: str, columns: dict[str, str]) -> None:
    """Best-effort schema migration for optional columns."""
    kind = _backend_kind()
    cur = conn.cursor()
    try:
        if kind == "sqlite":
            cur.execute(_normalize_sql(f"PRAGMA table_info({table_name})"))
            rows = cur.fetchall() or []
            existing = set()
            for row in rows:
                if isinstance(row, Mapping):
                    existing.add(str(row.get("name", "")).lower())
                else:
                    try:
            ...
            for col_name, col_type in columns.items():
                if col_name.lower() not in existing:
                    cur.execute(_normalize_sql(f"ALTER TABLE {table_name} ADD COLUMN {col_name} {col_type}"))
        else:
            for col_name, col_type in columns.items():
                cur.execute(_normalize_sql(f"ALTER TABLE {table_name} ADD COLUMN IF NOT EXISTS {col_name} {col_type}"))
    finally:
        cur.close()
...
def create_order(
    ...
    *,
    ...
    if qty <= 0:
        raise ValueError("qty must be positive")
    order_id = f"ord_{_hash_payload({'symbol': symbol, 'side': side, 'qty': qty, 'limit_price': limit_price, 'stop_price': stop_price, 'target_1': target_1, 'target_2': target_2, 'notes': notes, 'time': _utc_now()})}"
    now = _utc_now()
    with _connect() as conn:
        cur = conn.cursor()
        cur.execute(
            _normalize_sql(
                """
                INSERT INTO orders
                (order_id, account_id, symbol, side, qty, order_type, limit_price, stop_price, target_1, target_2, status, source, signal_hash, notes, created_at, updated_at, filled_price, filled_qty, filled_at, broker_ref)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, NULL, ?)
                """
            ),
            (order_id, account_id, symbol, side, qty, order_type, limit_price, stop_price, target_1, target_2, status, source, signal_hash, notes, now, now, broker_ref),
        )
        cur.close()
    record_event("order_created", {"order_id": order_id, "symbol": symbol, "side": side, "qty": qty, "status": status, "target_1": target_1, "target_2": target_2}, account_id=account_id, symbol=symbol)
    return order_id
