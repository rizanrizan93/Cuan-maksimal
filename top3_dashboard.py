...
def calculate_next_leader_score(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    story = _score(row.get("story_runway_score"))
    conversion = _score(row.get("financial_conversion_score"))
    alignment = _score(row.get("issuer_alignment_score"))
    sector = _score(row.get("sector_leadership_score"), 50)
    ...
    blocked = str(row.get("emir_decision_state") or "") in BLOCKED_STATES or str(row.get("fundamental_state") or "") == "FUNDAMENTAL_WEAK"
    eligible = bool(not blocked and fundamental_cov >= 35 and np.isfinite(_num(row.get("fundamental_conversion_score"))))
    if not eligible:
...
__all__ = [
    "BLOCKED_STATES",
    "FACTOR_FIELDS",
    "calculate_dashboard_scores", "calculate_next_leader_score",
    "enrich_dashboard_scores", "select_next_leaders",
    "select_top3",
    "recommendation_meta",
