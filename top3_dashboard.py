...
def calculate_dashboard_scores(row: Mapping[str, Any]) -> dict[str, float]:
    ...
    silent_accum = _weighted(
        [
            (row.get("holder_persistence_score"), 0.22),
            (row.get("inventory_dryness_score"), 0.18),
            (row.get("absorption_score"), 0.18),
            (row.get("close_acceptance20_pct"), 0.14),
            (100.0 - _score(row.get("distribution_score"), 100.0), 0.18),
            (row.get("broker_inventory_score"), 0.10),
        ],
...
def select_top3(radar: pd.DataFrame, limit: int = 3) -> pd.DataFrame:
    ...
    local = local.sort_values(
        ["_deep", "emir_final_score", "_priority", "dashboard_silent_accum_score", "dashboard_flow_score", "liquidity_score"],
...
def _plan_rows(row: Mapping[str, Any]) -> str:
    plan = [
        ...
        ("Breakout Trigger", "> " + _fmt_rupiah(row.get("trigger")), "trigger"),
        ("Stop Loss", "< " + _fmt_rupiah(row.get("stop_loss")), "stop"),
        ("TP1", _fmt_rupiah(row.get("tp1")), "target"),
        ("TP2", _fmt_rupiah(row.get("tp2")), "target"),
        ("Risk : Reward", f"1 : {_num(row.get('rr_tp2'), 0):.1f}" if _num(row.get("rr_tp2"), 0) > 0 else "—", "rr"),
    ]
    return "".join(
