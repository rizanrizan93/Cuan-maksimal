...
DATABASE_SCHEMA_VERSION = "emir_autonomous_schema_v6"
...
def database_status(config: DatabaseConfig) -> dict[str, Any]:
    return {
        ...
        "write_policy": "DATABASE_FIRST_EXACT_READBACK_REQUIRED",
    }
...
def test_connection(config: DatabaseConfig) -> pd.DataFrame:
    ...
    for table, select_column in checks.items():
        try:
            _request(config, "GET", table, params={"select": select_column, "limit": "1"})
    ...
    rows.insert(
        ...
        {
            **base,
            "state": "HEALTHY_EMIR_DATABASE_V6" if healthy else "DATABASE_NOT_READY_V6",
            "table": "__SUMMARY__",
        ...
        return pd.DataFrame(
            [{
                **base,
                ...
                "rows_written": 0,
                "state": "DATABASE_DISABLED",
                "detail": "Configure Streamlit Secrets. Scan results are not publishable without database commit.",
            }],
            columns=columns,
        )
...
__all__ = [
    ...
    "DatabaseConfig", "_headers", "config_from_mapping", "database_status", "persist_scan",
