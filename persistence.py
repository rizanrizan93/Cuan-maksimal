...
BRIDGE_VERSION = "1.3.1-v1.3.1"
DATABASE_SCHEMA_VERSION = "emir_public_framework_schema_v4"
SCANNER_VERSION = "1.3.1-supabase-service-role-grant-hotfix"
...
def test_connection(config: DatabaseConfig) -> pd.DataFrame:
    ...
    rows.insert(
        ...
        {
            **base,
            "state": "HEALTHY_EMIR_DATABASE_V4" if healthy else "DATABASE_NOT_READY_V4",
            "table": "__SUMMARY__",
...
def persist_scan(
    ...
    *,
    ...
    mode: str = "EMIR_HYBRID_400_TO_DEEP",
) -> pd.DataFrame:
    columns = [
    ...
    *,
    ...
    reports.insert(0, {
        **base,
        ...
        "verification_pct": round(pct, 1),
        "detail": f"{total_verified}/{total_expected} rows verified across 6 tables.",
    })
    return pd.DataFrame(reports, columns=columns)
