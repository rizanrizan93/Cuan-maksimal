...
def load_verified_outcome_memory(config: DatabaseConfig, *, limit: int = 5000, page_size: int = 500) -> pd.DataFrame:
    """Load verified outcome memory across prior scans for walk-forward calibration.

    Outcome rows are durable research evidence, so calibration must not depend on the
    user re-uploading the same CSV on every scan. Database errors fail soft to an empty
    frame; they never manufacture an outcome or downgrade a scan result.
    """
    columns = [
        "outcome_id", "scan_id", "ticker", "signal_date", "horizon_days",
        "outcome_verified", "return_pct", "max_drawdown_pct",
        "thesis_invalidated", "emir_lifecycle", "market_structure_mode",
    ]
    if not config.ready:
        return pd.DataFrame(columns=columns)
    rows: list[dict[str, Any]] = []
    safe_limit = max(1, int(limit))
    safe_page = max(1, min(int(page_size), 1000))
    for start in range(0, safe_limit, safe_page):
        try:
            response = _request(
                config, "GET", "cak_outcome_memory",
                params={
                    "select": "outcome_id,scan_id,ticker,signal_date,horizon_days,outcome_verified,payload",
                    "outcome_verified": "eq.true",
        ...
        if not isinstance(payload, list) or not payload:
            break
        for item in payload:
            record = item.get("payload") if isinstance(item.get("payload"), dict) else {}
            rows.append({
                "outcome_id": item.get("outcome_id"),
                "scan_id": item.get("scan_id"),
                "ticker": item.get("ticker") or record.get("ticker"),
                "signal_date": item.get("signal_date") or record.get("signal_date") or record.get("date"),
                "horizon_days": item.get("horizon_days") if item.get("horizon_days") is not None else record.get("horizon_days"),
                "outcome_verified": True,
                "return_pct": record.get("return_pct", record.get("return")),
                "max_drawdown_pct": record.get("max_drawdown_pct", record.get("max_drawdown")),
                "thesis_invalidated": record.get("thesis_invalidated", False),
                "emir_lifecycle": record.get("emir_lifecycle", record.get("lifecycle", "UNKNOWN")),
                "market_structure_mode": record.get("market_structure_mode", record.get("structure", "UNKNOWN")),
            })
    ...
    dedupe_cols = [c for c in ("outcome_id",) if c in frame.columns]
    if dedupe_cols:
        frame = frame.drop_duplicates(dedupe_cols, keep="first")
...
__all__ = [
    "BRIDGE_VERSION", "DATABASE_SCHEMA_VERSION", "SCANNER_VERSION", "DEFAULT_WRITE_CHUNK_SIZE", "load_verified_outcome_memory",
