...
"""Emir-owned ZAPI foreign-flow enrichment with bounded scoring influence.
...
ZAPI_FLOW_ENRICHMENT_VERSION = "1.1.0-emir-owned-idx-session-coverage"
...
OWNED_CACHE_URL = "https://raw.githubusercontent.com/rizanrizan93/Cuan-maksimal/main/data/zapi_foreign_flow_60d.csv.gz"
...
def _http_get(url: str, *, params: Mapping[str, Any] | None = None, api_key: str = "", timeout: float = 12.0, raw: bool = False) -> Any:
    ...
    try:
        ...
        response = curl_requests.get(url, params=dict(params or {}), headers=headers, timeout=timeout, impersonate="chrome")
    except Exception:
...
def _normalize_foreign_payload(payload: Any, trade_date: Any, universe: set[str]) -> pd.DataFrame:
    ...
    for item in rows:
        ...
        if not any(np.isfinite(value) for value in (buy, sell, net)):
            continue
        normalized.append({"ticker": ticker, "trade_date": day, "foreign_buy_shares": buy, "foreign_sell_shares": sell, "foreign_net_shares": net, "volume": _finite(item.get("volume"), np.nan), "value": _finite(item.get("value"), np.nan), "source": "ZAPI_IDX_FOREIGN_FLOW", "flow_unit": "SHARES"})
...
def _normalize_stock_summary_payload(payload: Any, trade_date: Any, universe: set[str]) -> pd.DataFrame:
    ...
    for item in rows:
        ...
        normalized.append({"ticker": ticker, "trade_date": day, "foreign_buy_shares": buy, "foreign_sell_shares": sell, "foreign_net_shares": net, "volume": _finite(item.get("Volume"), np.nan), "value": _finite(item.get("Value"), np.nan), "frequency": _finite(item.get("Frequency"), np.nan), "bid": _finite(item.get("Bid"), np.nan), "offer": _finite(item.get("Offer"), np.nan), "bid_volume": _finite(item.get("BidVolume"), np.nan), "offer_volume": _finite(item.get("OfferVolume"), np.nan), "listed_shares": _finite(item.get("ListedShares"), np.nan), "tradable_shares": _finite(item.get("TradebleShares"), np.nan), "source": "ZAPI_IDX_STOCK_SUMMARY_FALLBACK", "flow_unit": "SHARES"})
    return pd.DataFrame(normalized)


def _paginate_day(url: str, trade_date: Any, universe: Iterable[Any], *, api_key: str, normalizer: Any, page_size: int = 1000, max_pages: int = 6) -> pd.DataFrame:
    ...
    for _ in range(max(1, int(max_pages))):
        payload = _http_get(url, params={"date": day, "length": max(1, min(int(page_size), 1000)), "start": start}, api_key=api_key, timeout=12.0)
        root = _unwrap_payload(payload)
...
def _fetch_direct_day(universe: Iterable[Any], trade_date: Any, api_key: str) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        foreign = _paginate_day(ZAPI_FOREIGN_FLOW_URL, trade_date, universe, api_key=api_key, normalizer=_normalize_foreign_payload)
    ...
    try:
        fallback = _paginate_day(ZAPI_STOCK_SUMMARY_URL, trade_date, universe, api_key=api_key, normalizer=_normalize_stock_summary_payload)
        if not fallback.empty:
            return fallback, {"state": "OK_FALLBACK", "provider": "ZAPI_IDX_STOCK_SUMMARY_FALLBACK", "rows": len(fallback), "foreign_flow_error": foreign_error}
        return pd.DataFrame(), {"state": "NO_DATA", "provider": "ZAPI_DIRECT", "foreign_flow_error": foreign_error}
    except Exception as fallback_exc:
        return pd.DataFrame(), {"state": "FAIL_SOFT", "provider": "ZAPI_DIRECT", "foreign_flow_error": foreign_error, "fallback_error": f"{type(fallback_exc).__name__}: {str(fallback_exc)[:160]}"}


def _normalize_history(frame: pd.DataFrame | None) -> pd.DataFrame:
    columns = ["ticker", "trade_date", "foreign_buy_shares", "foreign_sell_shares", "foreign_net_shares", "volume", "value", "source", "flow_unit"]
    if not isinstance(frame, pd.DataFrame) or frame.empty:
        return pd.DataFrame(columns=columns)
    out = frame.copy()
    renames = {"date": "trade_date", "foreign_buy": "foreign_buy_shares", "foreign_sell": "foreign_sell_shares", "foreign_net": "foreign_net_shares", "net_foreign_shares": "foreign_net_shares"}
    ...
    for name in ("foreign_buy_shares", "foreign_sell_shares", "foreign_net_shares", "volume", "value", "bid", "offer", "bid_volume", "offer_volume"):
        if name in out.columns:
            out[name] = pd.to_numeric(out[name], errors="coerce")
    if "foreign_net_shares" not in out.columns:
        buy = pd.to_numeric(out["foreign_buy_shares"] if "foreign_buy_shares" in out.columns else pd.Series(np.nan, index=out.index), errors="coerce")
        sell = pd.to_numeric(out["foreign_sell_shares"] if "foreign_sell_shares" in out.columns else pd.Series(np.nan, index=out.index), errors="coerce")
    ...
    return out.sort_values(["ticker", "trade_date"], kind="stable").drop_duplicates(["ticker", "trade_date"], keep="last").reset_index(drop=True)
...
def _load_owned_cache() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        ...
        meta = {"state": "OK" if not frame.empty else "EMPTY", "provider": "EMIR_OWNED_ZAPI_CACHE", "rows": int(len(frame)), "latest_trade_date": (pd.to_datetime(frame["trade_date"], errors="coerce").max().date().isoformat() if not frame.empty else "")}
    except Exception as exc:
        frame = pd.DataFrame()
        meta = {"state": "FAIL_SOFT", "provider": "EMIR_OWNED_ZAPI_CACHE", "rows": 0, "error": f"{type(exc).__name__}: {str(exc)[:180]}"}
    ...
    if not fresh_enough and api_key:
        for day in recent:
            ...
            if not direct.empty:
                ...
                break
    combined_meta = {"state": "OK" if not history.empty else (direct_meta.get("state") or meta.get("state") or "NO_DATA"), "owned_cache_state": meta.get("state"), "owned_cache_provider": meta.get("provider"), "direct_state": direct_meta.get("state"), "direct_provider": direct_meta.get("provider"), "zapi_key_available": bool(api_key), "rows": int(len(history)), "latest_trade_date": (pd.to_datetime(history["trade_date"], errors="coerce").max().date().isoformat() if not history.empty else "")}
...
def score_foreign_history(
    ...
    *,
    ...
    for ticker in names:
        ...
        rows.append({"ticker": ticker, "zapi_foreign_latest_trade_date": latest_session, "zapi_foreign_observed_days": observed, "zapi_foreign_net_shares_1d": latest_net, "zapi_foreign_net_shares_5d": float(net5.sum(min_count=1)), "zapi_foreign_net_shares_10d": float(net10.sum(min_count=1)), "zapi_foreign_net_shares_20d": float(net20.sum(min_count=1)), "zapi_foreign_net_participation_1d": latest_net / latest_volume if np.isfinite(latest_net) and np.isfinite(latest_volume) and latest_volume > 0 else np.nan, "zapi_foreign_net_participation_5d": float(net5.sum(min_count=1) / vol5.sum(min_count=1)) if vol5.sum(min_count=1) > 0 else np.nan, "zapi_foreign_net_participation_10d": float(net10.sum(min_count=1) / vol10.sum(min_count=1)) if vol10.sum(min_count=1) > 0 else np.nan, "zapi_foreign_net_participation_20d": float(net20.sum(min_count=1) / vol20.sum(min_count=1)) if vol20.sum(min_count=1) > 0 else np.nan, "zapi_foreign_positive_days_ratio_5d": float(net5.gt(0).mean()) if net5.notna().any() else np.nan, "zapi_foreign_positive_days_ratio_20d": float(net20.gt(0).mean()) if net20.notna().any() else np.nan, "zapi_foreign_buy_ratio_20d": float(buy20.sum(min_count=1) / gross20.sum(min_count=1)) if gross20.sum(min_count=1) > 0 else np.nan, "zapi_foreign_buy_ratio_5d": float(pd.to_numeric(recent5.get("foreign_buy_shares"), errors="coerce").sum(min_count=1) / gross5.sum(min_count=1)) if gross5.sum(min_count=1) > 0 else np.nan, "zapi_flow_source": source_text, "zapi_flow_unit": "SHARES", "foreign_source": source_text, "foreign_latest_session": latest_session.date().isoformat(), "foreign_expected_sessions": 20, "foreign_expected_session_dates": expected_dates, "foreign_observed_sessions": observed, "foreign_coverage_ratio": coverage_ratio, "foreign_freshness_state": freshness_state, "foreign_provenance": provenance, "foreign_window_state": window_state, "_foreign_freshness_factor": freshness_factor})
    ...
    score20 = 0.55 * _cross_section_score(ratio20) + 0.45 * _ratio_abs_score(ratio20)
    score5 = 0.55 * _cross_section_score(ratio5) + 0.45 * _ratio_abs_score(ratio5)
    ...
    components = pd.concat([score20.rename("score20"), score5.rename("score5"), persistence20_score.rename("p20"), persistence5_score.rename("p5"), buy_score.rename("buy")], axis=1)
    ...
    out["zapi_accumulation_confirmation_score"] = (0.70 * flow_score + 0.20 * persistence20_score.fillna(50.0) + 0.10 * persistence5_score.fillna(50.0)).clip(0.0, 100.0).round(1)
    out["zapi_smart_money_confirmation_score"] = (0.75 * flow_score + 0.25 * persistence20_score.fillna(50.0)).clip(0.0, 100.0).round(1)
    out["zapi_smc_flow_confirmation_score"] = (0.80 * flow_score + 0.20 * score5.fillna(50.0)).clip(0.0, 100.0).round(1)
    ratio20 = pd.to_numeric(out["zapi_foreign_net_participation_20d"], errors="coerce")
    out["zapi_foreign_state"] = np.select([ratio20.ge(0.003), ratio20.le(-0.003)], ["NET_ACCUMULATION", "NET_DISTRIBUTION"], default="MIXED_NEUTRAL")
...
def enrich_super_universe(universe: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        rows.append({"flow_silent_accumulation_score": blended, "flow_silent_accumulation_confidence": coverage, "flow_silent_accumulation_data_coverage": coverage, "zapi_confirmation_weight_pct": weight_pct, "zapi_super_original_silent_score": base, "zapi_super_original_silent_coverage_pct": base_cov, "zapi_super_flow_basis": "PRICE_VOLUME_SILENT_ACCUM_PLUS_BOUNDED_ZAPI_FOREIGN_CONFIRMATION"})
