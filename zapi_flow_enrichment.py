...
def _normalize_foreign_payload(payload: Any, trade_date: Any, universe: set[str]) -> pd.DataFrame:
    ...
    for item in rows:
        ...
        normalized.append({"ticker": ticker, "trade_date": day, "foreign_buy_shares": buy, "foreign_sell_shares": sell, "foreign_net_shares": raw_net if np.isfinite(raw_net) else buy - sell, "volume": _finite(item.get("volume"), np.nan), "value": _finite(item.get("value"), np.nan), "source": "ZAPI_IDX_FOREIGN_FLOW", "flow_unit": "SHARES"})
...
def _normalize_stock_summary_payload(payload: Any, trade_date: Any, universe: set[str]) -> pd.DataFrame:
    ...
    for item in rows:
        ...
        normalized.append({"ticker": ticker, "trade_date": day, "foreign_buy_shares": buy, "foreign_sell_shares": sell, "foreign_net_shares": buy - sell, "volume": _finite(item.get("Volume"), np.nan), "value": _finite(item.get("Value"), np.nan), "frequency": _finite(item.get("Frequency"), np.nan), "bid": _finite(item.get("Bid"), np.nan), "offer": _finite(item.get("Offer"), np.nan), "bid_volume": _finite(item.get("BidVolume"), np.nan), "offer_volume": _finite(item.get("OfferVolume"), np.nan), "listed_shares": _finite(item.get("ListedShares"), np.nan), "tradable_shares": _finite(item.get("TradebleShares"), np.nan), "source": "ZAPI_IDX_STOCK_SUMMARY_FALLBACK", "flow_unit": "SHARES"})
    return pd.DataFrame(normalized)
...
def _load_shared_cache() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    try:
        ...
        meta = {"state": "OK" if not frame.empty else "EMPTY", "provider": "IDX_FLOW_SCANNER_SHARED_ZAPI_CACHE", "rows": int(len(frame)), "latest_trade_date": (pd.to_datetime(frame["trade_date"], errors="coerce").max().date().isoformat() if not frame.empty else "")}
    except Exception as exc:
        frame = pd.DataFrame()
        meta = {"state": "FAIL_SOFT", "provider": "IDX_FLOW_SCANNER_SHARED_ZAPI_CACHE", "rows": 0, "error": f"{type(exc).__name__}: {str(exc)[:180]}"}
    ...
    if not fresh_enough and api_key:
        for day in recent:
            ...
            if not direct.empty:
                ...
                break
    combined_meta = {"state": "OK" if not history.empty else (direct_meta.get("state") or meta.get("state") or "NO_DATA"), "shared_cache_state": meta.get("state"), "shared_cache_provider": meta.get("provider"), "direct_state": direct_meta.get("state"), "direct_provider": direct_meta.get("provider"), "zapi_key_available": bool(api_key), "rows": int(len(history)), "latest_trade_date": (pd.to_datetime(history["trade_date"], errors="coerce").max().date().isoformat() if not history.empty else "")}
...
def score_foreign_history(history: pd.DataFrame, universe: Iterable[Any] | None = None) -> pd.DataFrame:
    ...
    for ticker in names:
        ...
        rows.append({"ticker": ticker, "zapi_foreign_latest_trade_date": pd.to_datetime(latest.get("trade_date"), errors="coerce"), "zapi_foreign_observed_days": int(local["trade_date"].nunique()), "zapi_foreign_net_shares_1d": latest_net, "zapi_foreign_net_shares_5d": float(net5.sum(min_count=1)), "zapi_foreign_net_shares_20d": float(net20.sum(min_count=1)), "zapi_foreign_net_participation_1d": latest_net / latest_volume if np.isfinite(latest_net) and np.isfinite(latest_volume) and latest_volume > 0 else np.nan, "zapi_foreign_net_participation_5d": float(net5.sum(min_count=1) / vol5.sum(min_count=1)) if vol5.sum(min_count=1) > 0 else np.nan, "zapi_foreign_net_participation_20d": float(net20.sum(min_count=1) / vol20.sum(min_count=1)) if vol20.sum(min_count=1) > 0 else np.nan, "zapi_foreign_positive_days_ratio_5d": float(net5.gt(0).mean()) if net5.notna().any() else np.nan, "zapi_foreign_positive_days_ratio_20d": float(net20.gt(0).mean()) if net20.notna().any() else np.nan, "zapi_foreign_buy_ratio_20d": float(buy20.sum(min_count=1) / gross20.sum(min_count=1)) if gross20.sum(min_count=1) > 0 else np.nan, "zapi_foreign_buy_ratio_5d": float(pd.to_numeric(recent5.get("foreign_buy_shares"), errors="coerce").sum(min_count=1) / gross5.sum(min_count=1)) if gross5.sum(min_count=1) > 0 else np.nan, "zapi_flow_source": " | ".join(sorted(set(str(value) for value in local.get("source", pd.Series(dtype=str)).dropna() if str(value).strip()))), "zapi_flow_unit": "SHARES"})
