...
def _mixed_indentation_html() -> str:
    # Production regression: a wrapper-added style starts at column zero, so
    # global dedent cannot remove the four-space indentation of the legacy cards.
    return """<style>
.es-wrap{display:block}
</style>
    <div class="es-wrap">
        <section class="es-card rank1">
            <h2>TLDN</h2>
        </section>
    </div>"""
...
def test_normalize_top3_html_handles_mixed_indentation_from_runtime_wrappers() -> None:
    html = patch.normalize_top3_html(_mixed_indentation_html())
    nonempty = [line for line in html.splitlines() if line.strip()]

    assert html.startswith("<style>")
    assert '<div class="es-wrap">' in nonempty
    assert '<section class="es-card rank1">' in nonempty
    assert "<h2>TLDN</h2>" in nonempty
    assert all(line == line.lstrip() for line in nonempty)
...
def test_install_normalizes_final_dashboard_renderer(monkeypatch) -> None:
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: _mixed_indentation_html(),
    )
    ...
    assert html.startswith("<style>")
    assert all(line == line.lstrip() for line in html.splitlines() if line.strip())
