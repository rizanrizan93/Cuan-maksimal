...
def evaluate_breakout_setup(d: pd.DataFrame, fresh_bars: int = 3, max_age_bars: int = 20) -> dict:
    ...
    breakout_retest_ok = bool(
        ...
        and low <= breakout_reference + atr_v * 0.18
        and close > breakout_reference
        and close > open_
    )
    structure_ok = bool(breakout_reference > prev_high)

    valid = bool(breakout_retest_ok and structure_ok)
    ...
    entry_zone_low = float(max(0.0, breakout_reference - atr_v * 0.24))
    entry_zone_high = float(max(entry_zone_low, breakout_reference + atr_v * 0.06))
