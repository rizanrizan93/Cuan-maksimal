...
def main():
    ...
    job={
      'scan_id':'integration20','universe_hash':'h20','scanner_version':'1.6.1','status':'CREATED','current_stage':'BENCHMARK',
    ...
    artifact_dir = Path(__file__).resolve().parent / 'validation_artifacts_v1_6_2'
    artifact_dir.mkdir(exist_ok=True)
    export = radar.copy()
    export.insert(0, 'validation_data_mode', 'CONTROLLED_FIXTURE_NOT_LIVE_MARKET_DATA')
    export.to_csv(artifact_dir / 'TWENTY_TICKER_RADAR_SOURCE_V1_6_2.csv', index=False)
    summary = {
    ...
    (artifact_dir / 'TWENTY_TICKER_VALIDATION_V1_6_2.json').write_text(json.dumps(summary, indent=2, default=str))
