...
def evaluate_unicorn_setup(d: pd.DataFrame, fresh_bars: int = 3, max_age_bars: int = 20) -> dict:
    ...
    # Breaker / MSS proxy: candle close above EMA20 after sweep and above previous pivot high.
    ...
    mss_confirmed = bool(close > max(ema20 if np.isfinite(ema20) else close, breaker_bottom) and close > open_)
    structure_ok = bool((np.isfinite(ema20) and close >= ema20) or (np.isfinite(ema50) and close >= ema50))
    fvg_fresh = bool((len(d) - 1 - fvg_pos) <= max_age_bars)

    valid = bool(sweep_confirmed and mss_confirmed and structure_ok and fvg_fresh)
