...
def evaluate_unicorn_setup(d: pd.DataFrame, fresh_bars: int = 3, max_age_bars: int = 20) -> dict:
    ...
    fvg_span = max(fvg_top - fvg_bottom, atr_v * 0.12)
    entry_zone_low = float(max(0.0, fvg_bottom - atr_v * 0.10))
    entry_zone_high = float(max(entry_zone_low, min(fvg_bottom + fvg_span * 0.58, close - atr_v * 0.03)))
    entry_price = _entry_price_from_zone(entry_zone_low, entry_zone_high, bias=0.38)
