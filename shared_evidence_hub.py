...
This file is intentionally byte-for-byte aligned with the Pasticuan client.
Scanner-specific scoring, ranking, gates, and recommendations do not belong here.
"""

# The implementation is kept in one generated patch in Phase 5.6.  Importing
# scanner code here would couple the shared evidence identity to a scanner, so
...
def normalize_failure_reason(error: Exception | str) -> str:
    if isinstance(error, requests.Timeout): return MissingReason.TIMEOUT.value
    if isinstance(error, requests.ConnectionError): return MissingReason.CONNECTION_ERROR.value
    ...
    if text.startswith("HTTP_"):
        try: status = int(text.split("_", 1)[1])
        except ValueError: status = 0
        if status in {401, 403, 404, 429}: return f"HTTP_{status}"
        if status == 408: return MissingReason.TIMEOUT.value
        if 400 <= status < 500: return MissingReason.CONTEXT_REJECTED.value
        if status >= 500: return MissingReason.CONNECTION_ERROR.value
    if isinstance(error, (ValueError, TypeError, KeyError)): return MissingReason.PARSE_FAILURE.value
...
class HubConfig:
    ...
    def from_environment(cls, *, client_id: str) -> "HubConfig":
        ...
        try:
            timeout_seconds = max(2.0, min(60.0, float(_clean(os.getenv("SHARED_EVIDENCE_TIMEOUT_SECONDS", "20")))))
        except ValueError:
            timeout_seconds = 20.0
        return cls(url=url, key=key, key_type=key_type, client_id=_clean(client_id).upper() or "UNKNOWN", timeout_seconds=timeout_seconds)
    ...
    def status(self) -> dict[str, Any]:
        return {"hub_version": SHARED_EVIDENCE_HUB_VERSION, "state": "CONFIGURED" if self.ready else "MISSING", "url_state": secret_status(self.url if self.url else None), "key_state": secret_status(self.key if self.key else None), "key_type": self.key_type, "client_id": self.client_id, "idx_flow_blocked": _project_ref(self.url) == IDX_FLOW_SUPABASE_PROJECT_REF}

    def __repr__(self) -> str:
        return f"HubConfig(url='<configured>', key='<redacted>', key_type={self.key_type!r}, client_id={self.client_id!r}, ready={self.ready!r})"
...
class EvidenceKey:
    ...
    def normalized(self) -> "EvidenceKey":
        return EvidenceKey(_clean(self.provider).upper(), _clean(self.family).upper(), _clean(self.scope).upper(), self.target_date)

    def as_dict(self) -> dict[str, str]:
        key = self.normalized()
        return {"provider": key.provider, "evidence_family": key.family, "scope": key.scope, "target_date": key.target_date.isoformat()}
...
class SupabaseEvidenceBackend:
    ...
    def _headers(self, *, prefer: str = "") -> dict[str, str]:
        ...
        headers = {"apikey": self.config.key, "Authorization": f"Bearer {self.config.key}", "Accept": "application/json", "Content-Type": "application/json", "User-Agent": f"Shared-IDX-Evidence-Hub/{SHARED_EVIDENCE_HUB_VERSION}"}
        if prefer:
    ...
    def _request(self, method: str, path: str, *, params: Mapping[str, Any] | None = None, payload: Any = None, prefer: str = "") -> Any:
        response = self.session.request(method, f"{self.config.url}/rest/v1/{path.lstrip('/')}", params=dict(params or {}), json=payload, headers=self._headers(prefer=prefer), timeout=self.config.timeout_seconds)
    ...
    def upsert_rows(self, table: str, rows: Iterable[Mapping[str, Any]], *, conflict: Iterable[str]) -> list[dict[str, Any]]:
        ...
        payload = self._request("POST", table, params={"on_conflict": ",".join(conflict)}, payload=records, prefer="resolution=merge-duplicates,return=representation")
        return [dict(row) for row in payload] if isinstance(payload, list) else []
    ...
    def acquire_lease(self, key: EvidenceKey, holder: str, lease_seconds: int) -> Mapping[str, Any]:
        ...
        payload = self._rpc("evidence_acquire_refresh_lease", {"p_provider": normalized.provider, "p_family": normalized.family, "p_scope": normalized.scope, "p_target_date": normalized.target_date.isoformat(), "p_holder": holder, "p_lease_seconds": max(30, min(int(lease_seconds), 3600))})
        return dict(payload[0]) if isinstance(payload, list) and payload else {"acquired": False, "lease_state": "ERROR"}

    def complete_lease(self, key: EvidenceKey, holder: str, state: str = "COMPLETED") -> bool:
        normalized = key.normalized()
        payload = self._rpc("evidence_complete_refresh_lease", {"p_provider": normalized.provider, "p_family": normalized.family, "p_scope": normalized.scope, "p_target_date": normalized.target_date.isoformat(), "p_holder": holder, "p_result_state": _clean(state).upper() or "COMPLETED"})
        return bool(payload)

    def fail_lease(self, key: EvidenceKey, holder: str, reason: str) -> bool:
        normalized = key.normalized()
        payload = self._rpc("evidence_fail_refresh_lease", {"p_provider": normalized.provider, "p_family": normalized.family, "p_scope": normalized.scope, "p_target_date": normalized.target_date.isoformat(), "p_holder": holder, "p_reason": _clean(reason)[:160]})
        return bool(payload)

    def record_provider_state(self, row: Mapping[str, Any]) -> None:
        self.upsert_rows("evidence_provider_state", [row], conflict=("provider", "endpoint_family", "scope", "target_date"))


class SharedEvidenceCoordinator:
    def __init__(self, backend: EvidenceBackend, *, client_id: str, worker_id: str | None = None):
        self.backend, self.client_id = backend, _clean(client_id).upper() or "UNKNOWN"
        self.worker_id = worker_id or f"{self.client_id}:{uuid.uuid4()}"
        self._metrics_lock = Lock()
        self._metrics = {"calls_attempted": 0, "success": 0, "failure": 0, "cache_hits": 0, "cache_misses": 0, "calls_avoided": 0, "refresh_lock_collisions": 0}

    def metrics(self) -> dict[str, int]:
        with self._metrics_lock: return dict(self._metrics)

    def _inc(self, name: str) -> None:
        with self._metrics_lock: self._metrics[name] += 1

    @staticmethod
    def classify(rows: Iterable[Mapping[str, Any]], *, now: datetime | None = None, max_age: timedelta | None = None, minimum_rows: int = 1) -> EvidenceState:
        records = [dict(row) for row in rows]
        if not records: return EvidenceState.MISSING
        valid = [row for row in records if _clean(row.get("validation_state", "VALID")).upper() == "VALID"]
        if not valid: return EvidenceState.ERROR
        if len(valid) < max(1, int(minimum_rows)): return EvidenceState.INSUFFICIENT
        if max_age is not None:
            checked_at, stamps = now or datetime.now(timezone.utc), []
            for row in valid:
...
__all__ = ["EvidenceKey", "EvidenceState", "HubConfig", "IDX_FLOW_SUPABASE_PROJECT_REF", "MissingReason", "RefreshResult", "SecretStatus", "SHARED_EVIDENCE_HUB_VERSION", "SharedEvidenceCoordinator", "SupabaseEvidenceBackend", "normalize_failure_reason", "secret_status"]
