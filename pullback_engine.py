...
def evaluate_pullback_setup(d: pd.DataFrame, fresh_bars: int = 3, max_age_bars: int = 18) -> dict:
    ...
    mitigation_ok = bool(low <= support_anchor + atr_v * 0.28)
    ...
    entry_zone_low = float(max(0.0, support_anchor - atr_v * 0.28))
    entry_zone_high = float(max(entry_zone_low, support_anchor + atr_v * 0.10))
