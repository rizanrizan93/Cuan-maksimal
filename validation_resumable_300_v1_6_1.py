...
def main() -> None:
    ...
    def fetch_fundamental(_config, requested, **kwargs):
        ...
        rows = []
        for ticker in requested:
            item = {"ticker": ticker, "fundamental_state": "FUTURE_FUNDAMENTAL_SUPPORTIVE", "fundamental_coverage_pct": 80, "fundamental_conversion_score": 72, "revenue_growth_pct": 20, "earnings_growth_pct": 25}
    ...
    assert sum(len(chunk) for chunk in calls["KSEI"]) == 30
    assert sum(len(chunk) for chunk in calls["NEWS"]) == 30
    assert sum(len(chunk) for chunk in calls["FUNDAMENTAL"]) == 30
    assert max(len(chunk) for chunk in calls["KSEI"] + calls["NEWS"] + calls["FUNDAMENTAL"]) <= 20
