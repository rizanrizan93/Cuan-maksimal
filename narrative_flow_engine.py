...
ENGINE_VERSION = "1.3.1-supabase-service-role-grant-hotfix"
...
NARRATIVE_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "EARNINGS_CONVERSION": (
        ...
        "record high", "rekor", "cash flow", "free cash flow", "dividend", "dividen", "utilization",
    ),
    "OWNERSHIP_ALIGNMENT": (
...
def _truthy(value: Any) -> bool:
    return str(value or "").strip().lower() in {"true", "1", "yes", "y", "verified"}
...
def calculate_sector_context(fast: pd.DataFrame, universe: pd.DataFrame) -> dict[str, dict[str, Any]]:
    ...
    for sector, group in merged.groupby("sector"):
        breadth = 100 * pd.to_numeric(group["last_price"], errors="coerce").gt(pd.to_numeric(group["ema50"], errors="coerce")).mean()
        strength = pd.to_numeric(group.get("relative_strength60_pct"), errors="coerce").median()
        strength_momentum = pd.to_numeric(group["relative_strength_momentum_pct"], errors="coerce").median() if "relative_strength_momentum_pct" in group.columns else 0.0
        median_flow = pd.to_numeric(group["smart_money_score"], errors="coerce").median()
        score = _clip(
            0.30 * breadth + 0.25 * _clip(50 + 1.5 * _finite(strength, 0))
            + 0.20 * _clip(50 + 2.0 * _finite(strength_momentum, 0)) + 0.25 * _finite(median_flow, 50)
        )
        if _finite(strength, 0) >= 0 and _finite(strength_momentum, 0) >= 0:
            state = "LEADING"
        elif _finite(strength, 0) < 0 and _finite(strength_momentum, 0) >= 0:
            state = "IMPROVING"
        elif _finite(strength, 0) >= 0 and _finite(strength_momentum, 0) < 0:
            state = "WEAKENING"
        else:
            state = "LAGGING"
        coverage = _clip(25 * len(group))
...
def parse_idx_integrity(frame: pd.DataFrame | None, as_of: Any = None) -> dict[str, dict[str, Any]]:
    """Parse direct IDX board/HSC/regulatory/corporate-action evidence.

    This layer is deliberately conservative. Missing evidence never becomes a neutral score.
    """
    if frame is None or frame.empty:
        return {}
    local = frame.copy()
    ...
    for _, row in local.iterrows():
        ...
        listing_board = str(row.get("listing_board") or "").strip().upper()
        hsc = _truthy(row.get("hsc_flag"))
        special = _truthy(row.get("special_monitoring_flag"))
        fca = _truthy(row.get("full_call_auction_flag"))
        suspended = _truthy(row.get("suspension_flag"))
        uma = _truthy(row.get("uma_flag"))
        sanctions = _truthy(row.get("sanctions_flag"))
        corporate_action = _truthy(row.get("corporate_action_flag"))
        free_float = _finite(row.get("free_float_pct"), np.nan)
        over_1 = _truthy(row.get("over_1pct_disclosure_flag"))
        ...
        fields_present = [
            bool(listing_board), np.isfinite(free_float), "hsc_flag" in local.columns,
            "special_monitoring_flag" in local.columns or "full_call_auction_flag" in local.columns,
            "suspension_flag" in local.columns, pd.notna(observed), bool(str(row.get("source_url") or "").strip()),
        ]
        coverage = 100.0 * sum(fields_present) / len(fields_present) if verified else 0.0
        score = 100.0
        score -= 45.0 if hsc else 0.0
        score -= 55.0 if special or fca else 0.0
        score -= 90.0 if suspended else 0.0
        score -= 18.0 if uma else 0.0
        score -= 40.0 if sanctions else 0.0
        if np.isfinite(free_float):
            score -= 45.0 if free_float < 7.5 else 22.0 if free_float < 15.0 else 0.0
        score -= 12.0 if corporate_action else 0.0
        if np.isfinite(freshness_score):
            score = 0.85 * score + 0.15 * freshness_score
        score = _clip(score)
        hard_block_reasons: list[str] = []
        if suspended:
            hard_block_reasons.append("SUSPENDED")
        if special or fca:
            hard_block_reasons.append("SPECIAL_MONITORING_OR_FCA")
        if hsc:
            hard_block_reasons.append("HIGH_SHAREHOLDING_CONCENTRATION")
        if sanctions:
            hard_block_reasons.append("REGULATORY_SANCTION")
        if np.isfinite(free_float) and free_float < 7.5:
            hard_block_reasons.append("EXTREME_LOW_FREE_FLOAT")
        stale = not np.isfinite(age_days) or age_days > 60
        if stale:
            hard_block_reasons.append("STALE_OR_MISSING_IDX_EVIDENCE")
        hard_block = bool(hard_block_reasons)
        caution: list[str] = []
        if uma:
            caution.append("UMA_CAUTION")
        if np.isfinite(free_float) and 7.5 <= free_float < 15:
            caution.append("FREE_FLOAT_BELOW_15PCT")
        if corporate_action:
            caution.append("CORPORATE_ACTION_ACTIVE_OR_RECENT")
        if not over_1:
            caution.append("OWNERSHIP_ABOVE_1PCT_NOT_CONFIRMED")
        if not verified:
        ...
        output[ticker] = {
            ...
            "idx_integrity_caution_flags": " | ".join(caution) or "NONE",
            ...
            "hsc_flag": hsc,
            "special_monitoring_flag": special,
            "full_call_auction_flag": fca,
            "suspension_flag": suspended,
            "uma_flag": uma,
            ...
            "corporate_action_type": str(row.get("corporate_action_type") or "").strip().upper(),
            "corporate_action_effective_date": str(row.get("corporate_action_effective_date") or ""),
            "corporate_action_review_cleared": bool(verified and not corporate_action),
            "idx_integrity_source_url": str(row.get("source_url") or ""),
            "idx_integrity_note": str(row.get("note") or row.get("integrity_note") or ""),
        }
    return output
...
def build_execution_plan(
    ...
    return {
        ...
        "trigger_provenance": "DIRECT_BID_OFFER_EVIDENCE" if orderbook_verified else "OHLCV_STRUCTURE_PROXY_WAIT_DIRECT_BID_OFFER",
    }
...
def build_emir_profile(
    *, ticker: str, features: Mapping[str, Any], narrative: Mapping[str, Any] | None = None,
    ...
    narrative, broker, ownership, orderbook, market, sector, integrity = map(dict, (
        narrative or {}, broker or {}, ownership or {}, orderbook or {}, market or {}, sector or {}, integrity or {},
    ))
    ...
    conversion = _finite(narrative.get("financial_conversion_score"), np.nan)
    ...
    raw_score, evidence_coverage = _weighted_fixed([
        (flow_score, 0.21, flow_coverage),
        (structure_score, 0.16, structure_coverage),
        (narrative_runway_score, 0.15, narrative_runway_coverage),
        (sector_context_score, 0.10, sector_context_coverage),
        (alignment_owner_score, 0.10, alignment_owner_coverage),
        (orderbook_score, 0.07, orderbook_coverage),
        (trend, 0.07, 100 if np.isfinite(trend) else 0),
        (liquidity_float_score, 0.06, liquidity_float_coverage),
    ...
    orderbook_verified = str(orderbook.get("orderbook_provenance_state") or "") == "DIRECT_SOURCE_VERIFIED"
    integrity_verified = str(integrity.get("idx_integrity_provenance_state") or "") == "DIRECT_SOURCE_VERIFIED"
    ...
    core_thesis_ready = bool(
        ...
        and narrative_coverage >= 35 and _finite(conversion, 0) >= 45
        and int(narrative.get("narrative_verified_source_count", 0) or 0) >= 1
    ...
    thesis_ready = core_thesis_ready
    precise_ready = bool(
        thesis_ready and integrity_ready and orderbook_verified and orderbook_coverage >= 60
        and _finite(orderbook_score, 0) >= 55 and not capacity_block
    )

    if not deep_reviewed:
        state, action = "EMIR_RADAR_ONLY_NOT_DEEP_REVIEWED", "DEEP_REVIEW_REQUIRED"
    elif integrity_verified and integrity_hard_block:
    ...
    elif thesis_ready and not integrity_ready:
    ...
    if not orderbook_verified:
        risks.append("DIRECT_BID_OFFER_TRIGGER_MISSING")
    if not integrity_verified:
        risks.append("IDX_INTEGRITY_EVIDENCE_MISSING")
    ...
    position_cap = min(max(0.0, requested_cap), base_cap, max(0.0, liquidity_cap_pct)) if precise_ready else 0.0
    ...
    plan = build_execution_plan(features, precise_ready, lifecycle, orderbook)
    if thesis_ready and not integrity_ready:
        plan["execution_state"] = "CORE_THESIS_READY_WAIT_IDX_INTEGRITY"
    elif thesis_ready and integrity_ready and not orderbook_verified:
    ...
    return {
        ...
        "formula_provenance_state": "PUBLIC_IDEAS_PLUS_EMPIRICAL_IDX_HARDENING_NOT_OFFICIAL_CAK_FORMULA",
        **dict(features), **market, **sector, **narrative, **broker, **ownership, **orderbook, **integrity, **calibration,
        ...
        "production_ready": precise_ready,
...
__all__ = [
    ...
    "build_public_method_profile", "calculate_market_context", "calculate_market_features",
