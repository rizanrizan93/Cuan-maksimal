...
ENGINE_VERSION = "1.9.3-fundamental-join-integrity-fix"
...
def build_execution_plan(
    ...
    *, auto_eod_ready: bool = False,
    ...
    # Plan A — accumulation/pullback plan. Legacy entry/stop/TP fields remain this plan.
    ...
    accum_rr_tp1_trigger = _rr(trigger, tp1, stop)
    accum_rr_tp2_trigger = _rr(trigger, tp2, stop)

    # Plan B — breakout/retest plan. Do not reuse the accumulation stop/targets.
    ...
    result = {
        ...
        "execution_plan_semantics": "DUAL_PLAN_ACCUMULATION_AND_BREAKOUT_RETEST",
        "entry_low": entry_low, "entry_high": entry_high, "trigger": trigger, "stop_loss": stop,
        "tp1": tp1, "tp2": tp2,
        # Legacy midpoint RR retained for compatibility, but no longer presented as the only RR.
        "rr_tp1": round(_rr(entry_mid, tp1, stop), 2),
        "rr_tp2": round(_rr(entry_mid, tp2, stop), 2),
        ...
        "rr_tp1_at_trigger_using_accum_targets": round(accum_rr_tp1_trigger, 2) if np.isfinite(accum_rr_tp1_trigger) else np.nan,
        "rr_tp2_at_trigger_using_accum_targets": round(accum_rr_tp2_trigger, 2) if np.isfinite(accum_rr_tp2_trigger) else np.nan,
        ...
        "breakout_rr_tp1": round(_rr(breakout_entry, breakout_tp1, breakout_stop), 2) if breakout_valid else np.nan,
        "breakout_rr_tp2": round(_rr(breakout_entry, breakout_tp2, breakout_stop), 2) if breakout_valid else np.nan,
        "hard_stop_distance_pct": round(stop_distance_pct, 2),
        "risk_doctrine_state": "SEPARATE_ACCUMULATION_VS_BREAKOUT_RISK_GEOMETRY",
...
def build_emir_profile(
    *, ticker: str, features: Mapping[str, Any], narrative: Mapping[str, Any] | None = None,
    ...
    core_thesis_ready = bool(
        ...
        and fundamental_coverage >= 45 and _finite(fundamental_conversion, 0) >= 45
    ...
    auto_core_thesis_ready = bool(
        ...
        and fundamental_coverage >= 35 and _finite(fundamental_conversion, 0) >= 42
        and narrative_coverage >= 30 and _finite(conversion, 0) >= 45
        and (
            int(narrative.get("narrative_verified_source_count", 0) or 0) >= 1
            or int(narrative.get("narrative_independent_story_count", 0) or 0) >= 2
    ...
    elif _finite(fundamental_conversion, 0) < 42:
    ...
    data_quality = _finite(fundamental.get("fundamental_data_quality_score"), 0)
    ...
    manual_verified_story = int(narrative.get("narrative_verified_source_count",0) or 0) >= 1
    blockers=[]
    manual_conditions=[]
    ...
    cashflow_missing = ("MISSING" in cashflow_state or not cashflow_state)
    if cashflow_missing: manual_conditions.append("CASHFLOW_PROXY_OR_MANUAL_VERIFY")
    if freshness_state in {"STALE_QUARTERLY_PERIOD","UNKNOWN_PERIOD"}: blockers.append("FUNDAMENTAL_PERIOD_NOT_CURRENT")
    ...
    if not manual_verified_story: blockers.append("NO_VERIFIED_NARRATIVE_OR_OFFICIAL_EVENT")
    ...
    rr_high=_finite(plan.get("rr_tp1_at_entry_high"),np.nan); breakout_rr=_finite(plan.get("breakout_rr_tp1"),np.nan)
    accum_rr_ok = bool(np.isfinite(rr_high) and rr_high >= 1.5)
    breakout_rr_ok = bool(np.isfinite(breakout_rr) and breakout_rr >= 1.8)
    if not (accum_rr_ok or breakout_rr_ok): blockers.append("NO_EXECUTION_PATH_WITH_MIN_RR")
    real_money_candidate = len(blockers) == 0
    # Scanner-side READY remains strict. Proxy-only / missing-cashflow evidence is allowed
    # to score and become a manual candidate, but cannot self-authorize capital.
    direct_fundamental_ready = (official_cov >= 50 and not cashflow_missing)
    real_money_ready = bool(real_money_candidate and direct_fundamental_ready and integrity_ready and orderbook_verified and precise_ready)
    ...
    manual_cap = min(max(0.0,requested_cap), 5.0 if market_regime=="RISK_OFF" else 7.5) if real_money_candidate else 0.0
    if proxy_only or cashflow_missing:
    ...
    return {
        ...
        **dict(features), **market, **sector, **narrative, **broker, **ownership, **orderbook, **integrity, **fundamental, **calibration,
        ...
        "entry_authorization_state": "SCANNER_AUTHORIZED_DIRECT_VERIFIED" if real_money_ready else "MANUAL_CONFIRMATION_REQUIRED" if real_money_candidate else "NO_ENTRY_AUTHORIZATION",
        "pyramiding_state": "ADD_ONLY_AFTER_CONFIRMATION_NOT_AVERAGE_DOWN_BELOW_INVALIDATION",
        "production_ready": bool(real_money_ready if guarded_real_money else (precise_ready or auto_eod_ready)),
        "production_tier": ("GUARDED_DIRECT_VERIFIED" if real_money_ready else "MANUAL_CONFIRMATION_REQUIRED" if real_money_candidate else "NOT_READY") if guarded_real_money else ("DIRECT_PRECISE" if precise_ready else "AUTO_EOD_PROXY" if auto_eod_ready else "NOT_READY"),
