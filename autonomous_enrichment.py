...
def _period_freshness(period: Any, now: Any = None) -> tuple[str, float, float]:
    ...
    if age <= 245:
        return "AGING_QUARTERLY_PERIOD", 70.0, age
...
def fetch_yfinance_fundamental_snapshot(ticker: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        coverage = (
            0.34 * critical_completeness + 0.22 * statement_availability + 0.14 * growth_basis_quality
            + 0.12 * period_alignment_quality + 0.10 * period_freshness_quality + 0.08 * cashflow_quality
        )
        data_quality_score = _clip(
            0.28 * critical_completeness + 0.18 * statement_availability + 0.16 * period_alignment_quality
            + 0.14 * period_freshness_quality + 0.14 * cashflow_quality + 0.10 * growth_quality_score
        )
        ...
        return snapshot, {
            ...
            "detail": f"coverage={coverage:.1f}; data_quality={data_quality_score:.1f}; critical={critical_completeness:.1f}; statements={statement_availability:.1f}; growth_basis={growth_basis_state}; cashflow={cashflow_state}; alignment={period_alignment_state}; leverage={leverage_risk_state}; score_cap={score_cap:.1f}; official=0.0",
        }
    except Exception as exc:
...
__all__ = [
    ...
    "fetch_many_fundamentals", "fetch_many_ksei_profiles", "fetch_yfinance_fundamental_snapshot", "reconcile_fundamental_snapshot",
