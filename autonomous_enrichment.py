...
_AUTONOMOUS_MIN_INTERVAL_SECONDS = 0.28
...
def fetch_ksei_profile(ticker: str, timeout: int = 22, retries: int = 4) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    symbol = normalize_ticker(ticker)
    ...
    for attempt in range(max(1, retries)):
        try:
            ...
            if not ok and attempt + 1 < retries:
                # KSEI occasionally returns an HTML shell during throttling. Treat an empty
                # parse as transient, back off, and retry instead of converting it into a
                # durable 0%-coverage observation.
                time.sleep(min(8.0, 1.2 * (2 ** attempt) + random.uniform(0.2, 0.8)))
                continue
            ...
            last_error = exc
            retryable = any(token in str(exc) for token in ("403", "429", "500", "502", "503", "504", "timed out", "Timeout"))
...
def fetch_many_ksei_profiles(tickers: Iterable[str], max_workers: int = 2) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
def _cashflow_quality_state(ocf: float, fcf: float, ocf_conversion: float) -> tuple[str, float]:
    """Classify cash-flow *quality*, separately from evidence availability.

    Availability answers whether a statement exists. Quality answers whether operations
    are actually converting accounting profit into cash. Negative FCF alone is not a
    hard failure because capex can be intentional; negative OCF is materially different.
    """
    ocf_v = _finite(ocf, np.nan)
    fcf_v = _finite(fcf, np.nan)
    conv = _finite(ocf_conversion, np.nan)
    if not np.isfinite(ocf_v):
        return "CASHFLOW_NOT_AVAILABLE", 20.0
    if ocf_v < 0:
        if np.isfinite(fcf_v) and fcf_v < 0:
            return "OCF_AND_FCF_NEGATIVE", 10.0
        return "OCF_NEGATIVE", 15.0
    if np.isfinite(fcf_v) and fcf_v < 0:
        return "FCF_NEGATIVE_CAPEX_REVIEW", 45.0
    if np.isfinite(conv) and conv < 0.50:
        return "CASHFLOW_POSITIVE_WEAK_CONVERSION", 55.0
    if np.isfinite(conv) and conv >= 0.70 and (not np.isfinite(fcf_v) or fcf_v >= 0):
        return "CASHFLOW_POSITIVE_CONVERTING", 92.0 if np.isfinite(fcf_v) else 82.0
    return "CASHFLOW_POSITIVE_PARTIAL", 72.0
...
def recalibrate_cached_fundamental_snapshot(payload: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    cashflow_quality_state, cashflow_quality = _cashflow_quality_state(ocf, fcf, ocf_conversion)
    result["fundamental_cashflow_quality_state"] = cashflow_quality_state
...
def fetch_yfinance_fundamental_snapshot(ticker: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        cashflow_quality_state, cashflow_quality = _cashflow_quality_state(
            _finite(ocf_ttm, ocf), _finite(fcf_ttm, fcf), ocf_conversion
        )
...
def reconcile_fundamental_snapshot(proxy: Mapping[str, Any] | None, official: Mapping[str, Any] | None, *, now: Any = None) -> dict[str, Any]:
    ...
    cashflow_quality_state, cashflow_quality_score = _cashflow_quality_state(
        ocf, fcf, _finite(off.get("idx_official_ocf_conversion_ratio"), np.nan)
    )
    result["fundamental_cashflow_quality_state"] = cashflow_quality_state
    if np.isfinite(ocf):
        result["fundamental_cashflow_state"]=cash_state
        result["fundamental_cash_quality_score"] = cashflow_quality_score
    else:
    ...
    elif cashflow_quality_state in {"OCF_NEGATIVE", "OCF_AND_FCF_NEGATIVE"}: score_cap=min(score_cap,60.0)
    elif cashflow_quality_state == "FCF_NEGATIVE_CAPEX_REVIEW": score_cap=min(score_cap,78.0)
    elif cashflow_quality_state == "CASHFLOW_POSITIVE_WEAK_CONVERSION": score_cap=min(score_cap,76.0)
