...
STATE_PRIORITY = {
    ...
    "EMIR_FUNDAMENTAL_QUALITY_PENDING": 8,
...
def _is_property_like(row: Mapping[str, Any]) -> bool:
    sector = str(row.get("sector") or "").upper()
    industry = str(row.get("industry") or row.get("industry_name") or "").upper()
    return any(token in sector or token in industry for token in ("PROPERTY", "REAL ESTATE", "INDUSTRIAL ESTATE"))
...
def calculate_next_leader_score(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    growth_quality_state = str(row.get("fundamental_growth_quality_state") or "").upper()
    if "EARNINGS_BASE_EFFECT_EXTREME" in growth_quality_state:
        momentum = min(momentum, 72.0)
        quality_flags.append("EXTREME_EARNINGS_BASE_NORMALIZED")
    if "EARNINGS_SMALL_BASE" in growth_quality_state:
        momentum = min(momentum, 68.0)
        quality_flags.append("SMALL_EARNINGS_BASE_NORMALIZED")
    if "REVENUE_GROWTH_EXTREME_REVIEW" in growth_quality_state:
        momentum = min(momentum, 85.0)
        quality_flags.append("EXTREME_REVENUE_GROWTH_REVIEW")

    property_like = _is_property_like(row)
    if property_like:
        # Property/industrial-estate revenue is recognition/hand-over driven and lumpy.
        # Until presales/marketing-sales/backlog are directly modeled, generic YoY growth
        # cannot receive a perfect recurring-business momentum score.
        momentum = min(momentum, 90.0)
        model_state = "PROPERTY_LUMPY_RECOGNITION_NORMALIZED"
        quality_flags.append("PROPERTY_RECOGNITION_REQUIRES_PRESALES_CONFIRMATION")
    ...
    if cashflow_quality_state in {"OCF_NEGATIVE", "OCF_AND_FCF_NEGATIVE"}:
        penalty += 10.0
        quality_flags.append("OPERATING_CASH_FLOW_NEGATIVE")
    elif cashflow_quality_state == "FCF_NEGATIVE_CAPEX_REVIEW":
        penalty += 3.0
        quality_flags.append("FCF_NEGATIVE_CAPEX_REVIEW")
    ...
    elif future_ff_state == "FUTURE_FUNDAMENTAL_EVIDENCE_PENDING":
    ...
    if "REVENUE_GROWTH_EXTREME_REVIEW" in growth_quality_state:
        penalty += 2.0
    if property_like:
        rev_check = rev_ytd if q_count >= 2 and np.isfinite(rev_ytd) else _num(row.get("revenue_growth_yoy_pct"), np.nan)
        earn_check = earn_ytd if q_count >= 2 and np.isfinite(earn_ytd) else _num(row.get("earnings_growth_yoy_pct"), np.nan)
        if (np.isfinite(rev_check) and abs(rev_check) >= 80.0) or (np.isfinite(earn_check) and abs(earn_check) >= 120.0):
            penalty += 2.0
    ...
    return {
        ...
        "next_leader_presentation_eligible": presentation_eligible,
...
def select_next_leaders(radar: pd.DataFrame, limit: int = 10) -> pd.DataFrame:
    ...
    presentation_mask = local.get(
        "next_leader_presentation_eligible",
        local.get("next_leader_eligible", pd.Series(False, index=local.index)),
    )
    local = local[presentation_mask.fillna(False)].copy()
    if local.empty:
        return local
    local = local.sort_values(["next_leader_score", "fundamental_conversion_score", "next_leader_future_fundamental_score", "story_runway_score", "sector_leadership_score", "liquidity_score"], ascending=[False, False, False, False, False, False], na_position="last").head(max(1, int(limit))).copy()
...
def calculate_real_money_candidate_score(row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not np.isfinite(rr):
        # Backward compatibility for old persisted scans; new v1.9.12 scans always carry
        # execution_rr_tp1 from one selected path.
        rr_values = [_num(row.get("breakout_rr_tp1"), np.nan), _num(row.get("rr_tp1_at_entry_high"), np.nan)]
...
def enrich_dashboard_scores(radar: pd.DataFrame, frames: Mapping[str, pd.DataFrame] | None = None) -> pd.DataFrame:
    ...
    local["dashboard_price_change_pct"] = [
        (_price_change(frames.get(str(ticker), pd.DataFrame())) if not frames.get(str(ticker), pd.DataFrame()).empty
         else _num(local.loc[idx].get("price_change_1d_pct"), np.nan))
        for idx, ticker in zip(local.index, local.get("ticker", pd.Series(dtype=str)))
    ]
    ...
    if not leader_ranked.empty:
        leader_ranked = leader_ranked.sort_values(["next_leader_score", "fundamental_conversion_score", "next_leader_future_fundamental_score", "story_runway_score", "sector_leadership_score", "liquidity_score"], ascending=[False, False, False, False, False, False], na_position="last")
...
def _reason_lines(row: Mapping[str, Any]) -> list[tuple[str, str]]:
    ...
    return [
        ("positive", "narasi kuat dan terkonversi") if narrative >= 70 else ("developing", "narasi mulai terbentuk") if narrative >= 50 else ("warning", "narasi masih lemah"),
        ("positive", "flow mendukung") if flow >= 65 else ("warning", "flow perlu konfirmasi"),
        ("positive", "akumulasi senyap terdeteksi") if silent >= 65 else ("warning", "silent accumulation belum kuat"),
        ("positive", "struktur bullish/terjaga") if structure >= 65 else ("warning", "struktur belum matang"),
        ("positive", "momentum positif") if momentum >= 60 else ("warning", "momentum belum meyakinkan"),
        ("positive", "future fundamental mendukung") if fundamental >= 65 else ("warning", "konversi future fundamental belum terbukti"),
    ]


def _reason_summary(items: list[tuple[str, str]]) -> str:
    counts = {"positive": 0, "developing": 0, "warning": 0}
    for state, _ in items:
        counts[state] = counts.get(state, 0) + 1
    return f"Positif {counts['positive']} • Developing {counts['developing']} • Warning {counts['warning']}"
...
def _card_html(row: Mapping[str, Any], rank: int) -> str:
    ...
    reason_summary = _reason_summary(reason_items)
    ...
    <section class="es-card {top_color}">
      ...
      <div class="es-grid-bottom">
        ...
        <div class="es-panel"><h3>RINGKASAN ALASAN</h3><div class="es-reason-summary">{_esc(reason_summary)}</div><ul class="es-reasons">{reasons}</ul></div>
...
def render_top3_dashboard_html(
    ...
    *,
    ...
    .es-reason-summary{{margin:0 0 8px;padding:6px 8px;border-radius:7px;background:#0a2233;color:#9fc6d8;font-size:9px;text-align:center}} .es-reasons{{list-style:none;margin:0;padding:0;font-size:11px}} .es-reasons li{{padding:4px 0;display:flex;gap:7px;align-items:flex-start}} .es-reasons li span{{font-weight:900;min-width:12px}} .es-reasons li.positive span{{color:#4aed94}} .es-reasons li.developing span{{color:#ffd85a}} .es-reasons li.warning span{{color:#ff9a4c}}
