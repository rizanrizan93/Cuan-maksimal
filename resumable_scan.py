...
PIPELINE_VERSION = "1.6.1-small-universe-integrity-hotfix"
...
def choose_shortlist(fast: pd.DataFrame, sector_map: Mapping[str, Mapping[str, Any]], scan_mode: str, deep_limit: int) -> list[str]:
    if fast.empty or "feature_state" not in fast.columns:
        return []
    ...
    eligible["sector_overlay"] = eligible["ticker"].map(lambda ticker: float(sector_map.get(ticker, {}).get("sector_leadership_score", 50)))
    eligible["emir_discovery_score"] += 0.05 * (eligible["sector_overlay"] - 50)
    if scan_mode == "EMIR_AUTONOMOUS_DEEP_REVIEW":
        return eligible.sort_values("emir_discovery_score", ascending=False).head(min(max(1, int(deep_limit)), len(eligible)))["ticker"].tolist()
    if scan_mode == "EMIR_FLOW_RADAR_ONLY":
        return []
    return eligible.sort_values("emir_discovery_score", ascending=False).head(min(max(1, int(deep_limit)), len(eligible)))["ticker"].tolist()
...
def process_next_job_step(config: DatabaseConfig, job: Mapping[str, Any], *, now: Any = None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any] | None]:
    ...
    if stage == "FAST_RANKING":
        ...
        shortlist = choose_shortlist(context["fast"], context["sector_map"], str(settings.get("scan_mode")), int(settings.get("deep_limit") or 30))
        chunk_no = int(job.get("current_chunk") or 0) + 1
        record_job_chunk(
...
__all__ = [
    "PIPELINE_VERSION", "compute_fast_context", "choose_shortlist", "process_next_job_step",
