...
def compute_fast_context_from_features(
    ...
    *,
    ...
    local_fast = fast.copy() if isinstance(fast, pd.DataFrame) else pd.DataFrame()
    if "ticker" not in local_fast.columns:
        local_fast["ticker"] = pd.Series(dtype=str)
    if not local_fast.empty:
        local_fast["ticker"] = local_fast["ticker"].astype(str)
        local_fast = local_fast.drop_duplicates("ticker", keep="last")
    # Preserve universe order and represent missing cached features explicitly.
    local_fast = universe[["ticker"]].merge(local_fast, on="ticker", how="left")
    if "feature_state" not in local_fast.columns:
        local_fast["feature_state"] = "INSUFFICIENT_HISTORY"
    local_fast["feature_state"] = local_fast["feature_state"].fillna("INSUFFICIENT_HISTORY")
    ...
    for _, row in local_fast.iterrows():
        last_date = pd.to_datetime(row.get("last_date"), errors="coerce")
        if pd.notna(last_date) and str(row.get("feature_state") or "") == "OK":
            reference_frames[str(row.get("ticker"))] = pd.DataFrame(index=[pd.Timestamp(last_date)])
    benchmark_freshness = assess_benchmark_freshness(
        benchmark,
        reference_frames,
        min_universe_count=required_reference_count,
    )
    benchmark_for_context = benchmark if benchmark_freshness.get("benchmark_usable") else pd.DataFrame()
    market_context = calculate_market_context(benchmark_for_context)
    universe_market_context = calculate_market_context_from_universe(local_fast)
    ...
    sector_map = calculate_sector_context(local_fast, universe)
    return {
        "fast": local_fast,
        "market_context": market_context,
        "sector_map": sector_map,
        "benchmark_freshness": benchmark_freshness,
        "benchmark_for_features": benchmark_for_context,
    }
...
def compute_fast_context(
    ...
    *,
    ...
    tickers = universe["ticker"].tolist()
    # Establish benchmark usability before computing relative-strength features.
    ticker_count = len(tickers)
    required_reference_count = max(
        1,
        min(20, ticker_count, max(5, int(math.ceil(0.70 * ticker_count)))) if ticker_count else 1,
    ...
    context = compute_fast_context_from_features(universe, fast, benchmark, as_of=as_of)
    # Use the exact pre-computation benchmark state rather than reconstructing it from feature dates.
    context["benchmark_freshness"] = benchmark_freshness
    context["benchmark_for_features"] = benchmark_for_features
    return context


def _load_fast_features_cache_first(
    ...
    *,
    ...
    audits = [feature_audit] if isinstance(feature_audit, pd.DataFrame) and not feature_audit.empty else []
    if missing:
        fallback_frames, ohlcv_audit = load_cached_ohlcv_frames(
            config, missing, period=period, now=now, completed_only=completed_only,
        )
        if isinstance(ohlcv_audit, pd.DataFrame) and not ohlcv_audit.empty:
            audits.append(ohlcv_audit)
        fallback_records=[]
        for ticker in missing:
            frame = fallback_frames.get(ticker, pd.DataFrame())
            features = calculate_market_features(frame, benchmark, as_of=now)
            fallback_records.append({"ticker": ticker, **features})
            if str(features.get("feature_state") or "") == "OK":
                source_rows.append(build_market_feature_cache_row(ticker, features, checked_at=now, last_scan_id=scan_id))
        fallback = pd.DataFrame(fallback_records)
        cached = pd.concat([cached, fallback], ignore_index=True, sort=False) if not cached.empty else fallback
    if not cached.empty and "ticker" in cached.columns:
        cached = cached.drop_duplicates("ticker", keep="last").reset_index(drop=True)
    return cached, fallback_frames, pd.concat(audits, ignore_index=True, sort=False) if audits else pd.DataFrame(), source_rows
...
def _process_cache_stage(
    ...
    *,
    ...
    if stage == "BENCHMARK":
        ...
        benchmark_frames, _ = load_cached_ohlcv_frames(
        ...
        success_tickers = set(feature_hits)
        audits = [feature_audit] if isinstance(feature_audit, pd.DataFrame) and not feature_audit.empty else []
        if missing:
            frames, ohlcv_audit, cache_rows_ohlcv = fetch_ohlcv_cache_first(
                config, missing, period=str(settings.get("period") or "5y"),
            ...
            for ticker, frame in frames.items():
                ...
                features = calculate_market_features(frame, benchmark, as_of=now)
                if str(features.get("feature_state") or "") == "OK":
                    cache_rows_source.append(build_market_feature_cache_row(ticker, features, checked_at=now, last_scan_id=str(job.get("scan_id"))))
        ...
        profiles, actions, audit, cache_rows_source = fetch_ksei_cache_first(
            config, chunk, max_workers=min(int(settings.get("workers") or 3), 2), now=now,
            force_refresh=bool(settings.get("force_cache_refresh", False)), last_scan_id=str(job.get("scan_id")),
        )
...
def process_next_job_step(config: DatabaseConfig, job: Mapping[str, Any], *, now: Any = None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any] | None]:
    ...
    if stage == "FAST_RANKING":
        ...
        benchmark = benchmark_frames.get("^JKSE", pd.DataFrame())
        fast_features, fallback_frames, load_audit, feature_rows = _load_fast_features_cache_first(
            config, universe, benchmark, period=str(settings.get("period") or "5y"), now=now,
            completed_only=bool(settings.get("completed_only", True)), scan_id=str(job.get("scan_id")),
        )
        context = compute_fast_context_from_features(universe, fast_features, benchmark, as_of=now)
        if feature_rows:
            persist_verify_cache_bundle(config, scan_id=str(job.get("scan_id")), source_rows=feature_rows)
...
def finalize_job(config: DatabaseConfig, job: Mapping[str, Any], *, now: Any) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    benchmark = benchmark_frames.get("^JKSE", pd.DataFrame())
    fast_features, frames, ohlcv_load_audit, fallback_feature_rows = _load_fast_features_cache_first(
        config, universe, benchmark, period=period, now=now, completed_only=completed_only, scan_id=scan_id,
    )
    if fallback_feature_rows:
        persist_verify_cache_bundle(config, scan_id=scan_id, source_rows=fallback_feature_rows)
    ...
    raw_outcomes = pd.concat(outcome_frames, ignore_index=True, sort=False) if outcome_frames else pd.DataFrame()
    if not raw_outcomes.empty:
        if "outcome_id" in raw_outcomes.columns:
            raw_outcomes = raw_outcomes.drop_duplicates("outcome_id", keep="last")
        else:
            dedupe = [column for column in ("ticker", "signal_date", "horizon_days") if column in raw_outcomes.columns]
            if dedupe:
    ...
    for _, fast_row in fast.iterrows():
        ...
        future_ff = calculate_future_fundamental(
            ticker=ticker, events=ticker_events, narrative=narrative, fundamental=fundamental_map.get(ticker),
            ownership=ownership_map.get(ticker), sector=sector_map.get(ticker), issuer_context=metadata_map.get(ticker), as_of=now,
        )
    ...
    result = {
        ...
        "cache_summary": cache_summary_frame,
        "scan_id": scan_id,
        "as_of": now,
        "database_ready": config.ready,
...
__all__ = [
    "PIPELINE_VERSION", "DEEP_REVIEW_SCOPES", "compute_fast_context", "compute_fast_context_from_features", "choose_shortlist", "process_next_job_step",
