...
class StrategyParams:
    ...
    stop_atr: float = 1.6
    target1_atr: float = 2.5
    target2_atr: float = 4.2
    entry_buffer_atr: float = 0.30
...
def _simulate_single_trade(
    ...
    for j in range(i + 1, min(len(df), i + params.max_hold_bars + 1)):
        ...
        if exit_i < len(df):
            ...
            try:
                low = _safe_float(df.iloc[i + 1 : exit_i + 1]["Low"].min())
                high = _safe_float(df.iloc[i + 1 : exit_i + 1]["High"].max())
                if np.isfinite(low):
                    min_low = min(min_low, low)
                if np.isfinite(high):
                    max_high = max(max_high, high)
    ...
    if not np.isfinite(min_low):
        min_low = min(entry_fill, stop_fill)
    if not np.isfinite(max_high):
        max_high = max(entry_fill, target_fill)
    ...
    mae_r = (min_low - entry_fill) / risk
    mfe_r = (max_high - entry_fill) / risk
...
def run_backtest(
    ...
    opportunity_count = 0
    setup_opportunities: dict[str, int] = {"Breakout": 0, "Pullback": 0, "Trend": 0}
    setup_trades: dict[str, int] = {"Breakout": 0, "Pullback": 0, "Trend": 0}
    ...
    for i in range(60, len(d) - 2):
        ...
        opportunity_count += 1
        setup_opportunities[setup_kind] = setup_opportunities.get(setup_kind, 0) + 1
        ...
        trades.append(trade)
        open_trade = trade
        setup_trades[setup_kind] = setup_trades.get(setup_kind, 0) + 1

    stats = trade_metrics(trades)
    fill_rate = float(len(trades) / max(opportunity_count, 1)) if opportunity_count else np.nan
    stats.update({
        "signal_count": int(opportunity_count),
        "fill_rate": fill_rate,
        "setup_opportunities": setup_opportunities,
        "setup_trade_counts": setup_trades,
        "setup_fill_rates": {k: float(setup_trades.get(k, 0) / max(setup_opportunities.get(k, 0), 1)) if setup_opportunities.get(k, 0) else np.nan for k in setup_opportunities},
    })
    return {
        "symbol": symbol,
        "valid": True,
...
def trade_metrics(trades: Iterable[Trade]) -> dict:
    ...
    mae = np.array([getattr(t, "mae_r", np.nan) for t in trades], dtype=float)
    mfe = np.array([getattr(t, "mfe_r", np.nan) for t in trades], dtype=float)
    wins = rs[rs > 0]
    losses = rs[rs < 0]
    equity = np.cumsum(rs)
    ...
    return {
        ...
        "fill_rate": 1.0 if len(trades) else np.nan,
        ...
        "avg_mae_r": float(np.nanmean(mae)) if np.isfinite(mae).any() else np.nan,
        "avg_mfe_r": float(np.nanmean(mfe)) if np.isfinite(mfe).any() else np.nan,
        "median_mae_r": float(np.nanmedian(mae)) if np.isfinite(mae).any() else np.nan,
        "median_mfe_r": float(np.nanmedian(mfe)) if np.isfinite(mfe).any() else np.nan,
        "best_r": float(rs.max()),
...
def optimize_params(
    ...
    for params in _iter_param_grid(param_grid):
        ...
        if not np.isfinite(exp_r) or not np.isfinite(pf) or not np.isfinite(dd) or not np.isfinite(wr):
            continue
        fill_rate = _safe_float(res.get("fill_rate"), np.nan)
        score = (exp_r * 100.0) + (pf * 8.0) + (wr * 30.0) - (dd * 12.0) + min(15.0, tc * 0.7)
        if np.isfinite(fill_rate):
            score += min(8.0, max(0.0, fill_rate * 8.0))
        ...
        rows.append(
            {
                ...
                "test_fill_rate": oos.get("fill_rate", np.nan),
...
def summarize_walk_forward(folds: pd.DataFrame) -> dict:
    ...
    test_fill = folds["test_fill_rate"].dropna() if "test_fill_rate" in folds else pd.Series(dtype=float)
    ...
    return {
        ...
        "avg_test_fill_rate": float(test_fill.mean()) if not test_fill.empty else np.nan,
